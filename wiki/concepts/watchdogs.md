...
**Reading a panic**: after a panic+reboot, check `/sys/fs/pstore` for `dmesg-*` files (the kernel's last words), and tail `/var/log/tower-remote.log` on [[book5]] for the syslog leading up to the crash.
